...
const AdminUserAttendancePage = () => {
    ...
    const {user, loading: userLoading, error: userError} = useUserData(token, id);
    const {list, loading: listLoading, error: listError} = useList(token, id, getUserAttendance);
    ...
    if (listLoading) pageContent = <p>Loading...</p>;
        ...
        />;    
        backLink = <p><Link to={`/admin/users/${id}`}>Back to user</Link></p>;
    }
